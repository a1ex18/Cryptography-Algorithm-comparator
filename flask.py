...
def run_rsa_with_progress(bits=512, msg_size=32, repeats=2):
    ...
    try:
        ...
        # Key generation
        emit_progress('rsa', 'keygen', 10, {'status': 'Generating prime p...'})
        time.sleep(0.1)
        t = time.perf_counter()
        ...
        emit_progress('rsa', 'prepare', 40, {'status': 'Message prepared', 'size': msg_size})
...
def run_ecc_with_progress(repeats=10):
...
def handle_run_benchmarks(data):
    """Run all algorithms in parallel"""
    
    # Start each algorithm in a separate thread
    threads = [
        threading.Thread(target=run_rsa_with_progress, args=(512, 32, 2)),
        threading.Thread(target=run_ecc_with_progress, args=(10,)),
        threading.Thread(target=run_sha256_with_progress, args=([16, 128, 1024], 50)),
        threading.Thread(target=run_bb84_with_progress, args=([128, 512], 5, 0.01))
    ]
    
    for thread in threads:
        thread.start()
    
    emit('benchmarks_started', {'message': 'All algorithms started'})
...
if __name__ == '__main__':
>>>>>>> 11b8125d4f42e445451817bfab8ab123ec0bae71
